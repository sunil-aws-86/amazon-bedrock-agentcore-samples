...
class TestMemoryIntegration:
    ...
    def mock_memory_tools(self):
        """Mock memory tools."""
        with patch('sre_agent.supervisor.SaveMemoryTool') as mock_save, \
    ...
    def mock_memory_hooks(self):
        ...
        with patch('sre_agent.supervisor.MemoryHookProvider') as mock_hooks_class:
            mock_hooks = Mock()
            mock_hooks.on_investigation_start = AsyncMock(return_value={
                "user_preferences": [],
    ...
    async def test_aggregate_responses_saves_summary(
        ...
        with patch.object(SupervisorAgent, '_create_llm') as mock_create_llm, \
            ...
            mock_formatter.format_investigation_response.return_value = "Investigation complete."
