...
class SREMemoryClient:
    ...
    def _initialize_memories(self):
        ...
        try:
            ...
            if existing_memory and not self.force_delete:
                ...
                logger.info(f"Using existing memory: {self.memory_id} (name: {existing_memory['name']})")
                logger.info(f"Memory status: {existing_memory.get('status', 'unknown')}")
                ...
                if strategy_count >= 3:  # We expect 3 strategies
                    logger.info(f"Found {strategy_count} existing strategies - memory is already configured")
                    # Check if all strategies are ACTIVE
                    creating_count = sum(1 for s in existing_strategies if s.get('status') == 'CREATING')
                    if creating_count > 0:
                        logger.warning(f"{creating_count} strategies are still in CREATING state - memory system may not be fully operational")
                    return  # Memory is already configured
                else:
                    logger.info(f"Found {strategy_count} strategies, expected 3 - will add missing ones")
                ...
                if existing_memory and self.force_delete:
                    logger.warning(f"Force delete enabled - deleting existing memory: {existing_memory['id']}")
                    try:
            ...
            if "infrastructure_knowledge" not in existing_names:
                ...
                logger.info("Infrastructure knowledge strategy already exists, skipping")
    ...
    def save_event(
        ...
        try:
            ...
            logger.info(f"=== SAVE_EVENT TRACE START ===")
            ...
            logger.info(f"Calling create_event with:")
            logger.info(f"  memory_id: {self.memory_id}")
            logger.info(f"  actor_id: {actor_id}")
            logger.info(f"  session_id: {session_id}")
            ...
            logger.info(f"=== SAVE_EVENT TRACE END ===")
            logger.info(f"Saved {memory_type} event for {actor_id} (event_id: {event_id})")
            logger.info(f"Event data size: {len(str(event_data))} characters")
            return True
            
        except Exception as e:
            logger.error(f"Failed to save {memory_type} event for {actor_id}: {e}", exc_info=True)
            return False
    ...
    def retrieve_memories(
        ...
        try:
            ...
            logger.info(f"Retrieving {memory_type} memories: actor_id={actor_id}, namespace={namespace}, query='{query}'")
            ...
            logger.error(f"Failed to retrieve {memory_type} memories for {actor_id}: {e}", exc_info=True)
            return []
    ...
    def get_all_events(
        ...
        """Get all events for a specific actor using list_events API."""
        if not self.memory_id:
            logger.warning("Memory system not initialized, returning empty results")
            return []
        ...
        try:
            ...
            for memory in memories:
                ...
                if memory_id.startswith(f"{self.memory_name}-"):
                    logger.info(f"Found existing memory: {memory_id} with status {memory.get('status')}")
                    ...
                    try:
                        ...
                        cp_client = MemoryControlPlaneClient(region_name=self.client.gmcp_client._client_config.region_name)
            ...
            logger.info(f"No existing memory found with name prefix: {self.memory_name}")
            return None
    ...
    def clear_memory(
        ...
        if not self.memory_id:
            logger.warning("Memory system not initialized, cannot clear")
            return False
            
        try:
            # Note: The AgentCore Memory API doesn't have direct clear methods
            # This would need to be implemented by deleting individual events
            # or recreating the memory resource
            logger.warning("Memory clearing not implemented - would require event deletion")
            return True
