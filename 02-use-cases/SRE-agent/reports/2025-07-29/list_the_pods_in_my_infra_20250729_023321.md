...
- **Severity**: High - Critical database component failing to start, likely affecting data access for other services
...
### 🚨 Critical Alerts
- Database pod repeatedly crashing may lead to data unavailability for dependent services
...
Based on the `get_pod_status` tool results, I can see that most pods are running normally, but there's one pod (`database-pod-7b9c4d8f2a-x5m1q`) in CrashLoopBackOff state that might need attention.
